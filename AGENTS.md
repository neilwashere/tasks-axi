...
- **GitHub adoption is explicit.** `inbox` discovers Project items with no Task ID, while `adopt` assigns one only after checking exact identity and any existing item assignment; it never wraps an existing product issue in a fleet-only issue.
...
- **Write ops are confirmation-forward.** Every mutation (`adopt`/`add`/`start`/`done`/`cancel`/`reopen`/`update`/`rm`/`block`/`unblock`/`hold`/`unhold`/`owner-transfer`/`public-followup`/`mv`/`prune`/`render`) leads with a terse `ok:` line (built in `confirm.ts`) confirming the write result.
